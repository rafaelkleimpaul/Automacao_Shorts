...
@app.api_route("/serve/{job_id}/{filename}", methods=["GET", "HEAD"], tags=["ops"])
def serve_file(job_id: str, filename: str, request: Request) -> FileResponse:
    """
...
def purge_request_endpoint(request: Request, _auth: None = Depends(verify_api_key), _rate: None = Depends(_admin_rate_limit)) -> dict[str, Any]:
    ...
    The buttons call /purge_used_broll/confirm and /purge_used_broll/cancel with a
    time-limited token (2h). Requires PUBLIC_BASE_URL env var (e.g. http://1.2.3.4:8000)
    so the inline-keyboard buttons point to this service.
    """
    import os
    from pipeline.stock_monitor import request_purge_approval
    base_url = os.environ.get("PUBLIC_BASE_URL", str(request.base_url).rstrip("/"))
    return request_purge_approval(base_url)


@app.get("/purge_used_broll/confirm", tags=["ops"])
@app.post("/purge_used_broll/confirm", tags=["ops"])
def purge_confirm_endpoint(token: str) -> dict[str, Any]:
    """
    Confirm and execute a previously requested b-roll purge.
    Accepts GET (Telegram button click) and POST (n8n / curl).
    Token must match one generated by /purge_used_broll/request and not be expired (2h).
    """
    from pipeline.stock_monitor import confirm_purge
    return confirm_purge(token)


@app.get("/purge_used_broll/cancel", tags=["ops"])
@app.post("/purge_used_broll/cancel", tags=["ops"])
def purge_cancel_endpoint(token: str) -> dict[str, Any]:
    """Cancel a pending purge request. Accepts GET (Telegram button) and POST."""
    from pipeline.stock_monitor import cancel_purge
    return cancel_purge(token)
